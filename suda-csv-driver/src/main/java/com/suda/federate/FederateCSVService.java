...
import static com.suda.federate.CSVUtils.*;
...
public class FederateCSVService extends FederateDBService {
    ...
    public void publicRangeCount(FederateService.SQLExpression request, StreamObserver<FederateService.SQLReply> responseObserver) {
        ...
        result = localRangeCount(request.getPoint(), request.getTable(),
                request.getDoubleNumber());
        //构造返回
        FederateService.SQLReply reply =
                FederateService.SQLReply.newBuilder().setIntegerNumber(result).build();
    ...
    public void publicRangeQuery(FederateService.SQLExpression request, StreamObserver<FederateService.SQLReply> responseObserver) {
        ...
        try {
            List<FederateCommon.Point> res =
                    localRangeQuery(request.getPoint(), request.getTable(),
                            request.getDoubleNumber());
    ...
    @Override
    public void knnRadiusQuery(FederateService.SQLExpression request,
                               StreamObserver<FederateService.SQLReply> responseObserver) {
        System.out.println("收到的信息：" + request.getFunction());
        Double result = 0.0;

        int k = request.getIntegerNumber();;
        result = localKnnRadiusQuery(request.getPoint(), request.getTable(), k);

        //构造返回
        FederateService.SQLReply reply =
                FederateService.SQLReply.newBuilder().setDoubleNumber(result).build();
    ...
    public void privacyPolygonRangeQuery(FederateService.SQLExpression request, StreamObserver<FederateService.Status> responseObserver) {
        ...
        try {
            int K = (new Double(request.getDoubleNumber())).intValue();
