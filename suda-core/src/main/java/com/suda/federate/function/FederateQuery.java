...
public abstract class FederateQuery {
    ...
    protected static Integer privacySummation(FederateService.SQLExpression expression) {
       ...
       //  }
        ...
        int leaderIndex = 0;
        FederateDBClient leaderClient = federateDBClients.get(leaderIndex);
        FederateService.SummationResponse response = leaderClient.localSummation(requestBuilder.setNowIndex(leaderIndex).setEndIndex(federateDBClients.size() - 1).build());
        fakeLocalSumList.addAll(response.getFakeLocalSumList().stream().map(x -> x.getNumList()).collect(Collectors.toList()));
